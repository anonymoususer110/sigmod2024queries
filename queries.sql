--- SQL queries which follow MonetDB's SQL dialect


-- Q1 (get_population_stats_enhanced)
SELECT t.state_short, sum(t.crime_index) AS crime_index
FROM get_population_stats_table(
    (SELECT total_population, total_adult_population, number_of_robberies, replace_o_a(slice(to_lower(strip(state_full)))) AS state_short
    FROM us_cities_tiny WHERE total_population > 500000)
) AS t
GROUP BY t.state_short;

-- Q2 (zillow)

SELECT sum(bathrooms) AS sum_ba, sum(sqft) AS sum_sqft, count(url) AS urls, count(offer) AS offers, count(zip_code) AS zip_codes
FROM 
    (
        SELECT t.bedrooms, extract_ba(t.facts_and_features) AS bathrooms, extract_sqft(t.facts_and_features) AS sqft,
               extract_pcode(t.postal_code) AS zip_code, replace_o_a(strip(to_lower(url))) AS url, extract_offer(title) AS offer
        FROM (SELECT extract_bd(facts_and_features) AS bedrooms, extract_price(price) AS price_n, * FROM zillow_large) AS t
...
SELECT replace_o_a(slice(to_lower(strip(o_comment)))) FROM orders;
...
SELECT 
replace_o_a(slice(to_lower(strip(t.attr1)))),
...
SELECT t.state_short, sum(t.crime_index) AS crime_index
FROM get_population_stats_table(
    (SELECT total_population, total_adult_population, number_of_robberies, slice(to_lower(state_full)) AS state_short
    FROM us_cities_tiny WHERE total_population > 500000)
) AS t
GROUP BY t.state_short;
...
SELECT clean_incident_zips(incident_zip) FROM requests_311;
...
FROM
(
    SELECT extract_bd(facts_and_features) AS bedrooms, price FROM zillow
) AS T
WHERE T.bedrooms < 10
GROUP BY T.bedrooms;
...
SELECT
sum_int(to_int(strip_commas(strip_dollar_sign(price)))),
...
(
    SELECT
    extract_bd(facts_and_features) AS bedrooms, *
    FROM zillow
) AS T
WHERE T.bedrooms < 10
GROUP BY T.bedrooms;
...
SELECT charcount(lower(facts_and_features)) FROM zillow;

-- Q15

SELECT tokencount(tokens(facts_and_features)) FROM zillow;
